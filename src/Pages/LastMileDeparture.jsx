...
import { lastMileDepartureAPI, locationMasterAPI, driverMasterAPI } from "../utils/Api";
...
export default function LastMileDeparturePage() {
  ...
  const fetchLocations = useCallback(async () => {
    try {
      ...
      // Use Sideing locations as plant options for Last Mile Departure
  ...
  const validateVINsDebounced = useCallback(async (vinDetails) => {
    ...
    try {
      const validation = await lastMileDepartureAPI.validateVINs(vinArray);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await lastMileDepartureAPI.createLastMileDeparture(form);
      setSaved(true);
      showToast("Last Mile Departure created successfully!", 'success');
      setTimeout(() => {
        setSaved(false);
        setLoading(false);
      }, 3500);
    } catch (error) {
      console.error("Error creating Last Mile Departure:", error);
      console.error("Error response:", error.response);
      
      // Show detailed error message from backend
      const errorMessage = error.response?.data?.error || error.message || "Failed to create Last Mile Departure record";
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Last Mile Departure Entry</h2>
          <p className="text-gray-600 mt-1">
            Register vehicle departure for final delivery with dispatch and driver details
          </p>
        </div>
      ...
      {saved && (
        <div className="mb-5 flex items-center gap-3 px-5 py-3 rounded-lg bg-green-50 border border-green-200 text-green-800">
          ...
          <span className="text-sm font-medium">
            Last Mile Departure entry saved successfully. Vehicle dispatched for final delivery.
          </span>
        </div>
      )}
              ...
              <div>
                <FieldLabel required>
                  ...
                  <span className="ml-1 text-gray-400 normal-case tracking-normal font-normal text-xs">
                    (Only VINs from completed OEM pickups allowed)
                  </span>
                </FieldLabel>
                <div className="relative">
                  <textarea
                    ...
                    {vinValidation.isValid ? (
                      ...
                      <div className="text-red-700 bg-red-50 border-red-200 p-2 rounded">
                        ...
                        <p className="text-xs mt-2">
                          Available VINs from completed OEM pickups:{" "}
